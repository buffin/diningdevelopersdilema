...
public class TimelineBuilderTest {
	...
	/**
	 * User votes (locationX/locationY): A: --->
	 */
	@Test
	...
	/**
	 * User votes (locationX/locationY): A: ---> B: --->
	 */
	@Test
	...
	/**
	 * User votes (locationX/locationY): A: ---(100/000)--->
	 * 
	 * =>
	 * 
	 * Location timelines: 0 X: 100
	 */
	@Test
	public void singleVote() {
		...
		assertThat(series.getData(), is(expect(0, 100)));
	}

	/**
	 * User votes (locationX/locationY): A: ---(100/000)---------------> B:
	 * ---------------(025/075)--->
	 * 
	 * =>
	 * 
	 * Location timelines: -1 0 X: 100 125 Y: --- 075
	 * 
	 */
	@Test
	public void twoVotes() {
		...
		assertThat(seriesX.getData(), is(expect(-1, 100, 0, 125)));
		ChartSeries seriesY = seriesIter.next();
		assertThat(seriesY.getLabel(), is(LOCATION_Y_NAME));
		assertThat(seriesY.getData(), is(expect(0, 75)));
	}
	 ...
	 * =>
	 * 
	 * Location timelines: -2 -1 0 X: 080 090 060 Y: 020 110 140
	 * 
	 */
	@Test
	public void threeVotes() {
		...
		ChartSeries seriesX = seriesIter.next();
		assertThat(seriesX.getLabel(), is(LOCATION_X_NAME));
		assertThat(seriesX.getData(), is(expect(-2, 80, -1, 90, 0, 60)));
		ChartSeries seriesY = seriesIter.next();
		assertThat(seriesY.getLabel(), is(LOCATION_Y_NAME));
		assertThat(seriesY.getData(), is(expect(-2, 20, -1, 110, 0, 140)));
	}

	/**
	 * Votes: A: ---(000/100)---> B: ---(100/000)--->
	 * 
	 * =>
	 * 
	 * Timeline series: 0 1 A: 100 100 B: 000 100
	 * 
	 * To keep TimelineBuilder logic simple, this case is not treated precisely
	 * correct. Instead, timestep one is expanded to two timeline entries. Note:
	 * In reality two simultaneous votes will hardly occur.
	 */
	@Test
	public void twoSimultaneousVotes() {
		...
		ChartSeries seriesX = seriesIter.next();
		assertThat(seriesX.getLabel(), is(LOCATION_X_NAME));
		assertThat(seriesX.getData(), is(expect(-1, 100, 0, 100)));
		ChartSeries seriesY = seriesIter.next();
		assertThat(seriesY.getLabel(), is(LOCATION_Y_NAME));
		assertThat(seriesY.getData(), is(expect(0, 100)));
	}

	private TimelineVoteModel vote(TimelineLocationModel location, int points,
			int timestep) {
	...
	private static Map<Object, Number> expect(Integer... keysAndValues) {
		assertTrue("Even number of keys and values required",
